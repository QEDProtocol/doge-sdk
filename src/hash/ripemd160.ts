...
class RIPEMD160Hasher implements ISimpleHasher {
  ...
  update(inputData: Uint8Array | string, encoding?: ValidHashEncodings): this {
    if (this.finalized) throw new Error('Digest already called');
    const data = decodeData(inputData, encoding);
  ...
  digest(encoding?: ValidHashEncodings): Uint8Array | string {
    if (this.finalized) throw new Error('Digest already called');
    this.finalized = true;
    ...
    return encodeData(new Uint8Array(res.buffer), encoding);
  }
}
...
function ripemd160Buffer(data: Uint8Array | Int8Array | Uint16Array | Int16Array | Uint32Array | Int32Array | BigUint64Array | BigInt64Array | ArrayBuffer, encoding?: string): Uint8Array | string {
  const inputDataBuffer = data instanceof ArrayBuffer ? data : data.buffer;

  const hash = new RIPEMD160Hasher();
...
function ripemd160Hex(hexString: string, encoding?: string) {
